...
// Add eto sterlization store entry
...
export const getETOSterlizationById = async (req, res) => {
  try {
    ...
    if (!etoSterlizationEntry) {
      return res.status(404).json({message: 'ETOSterlization entry not found'});
    }
...
export const deleteETOSterlizationById = async (req, res) => {
  try {
    ...
    const deletedETOSterlization = await ETOSterlization.findByIdAndDelete(id);
