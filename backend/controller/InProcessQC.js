...
export const addInProcessQC = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({
        message: 'InProcessQC entry added successfully',
        data: savedInProcessQC,
      });
  } catch (error) {
...
export const getAllInProcessQC = async (req, res) => {
  try {
    const InProcessQCEntries = await InProcessQC.find();
    res
      .status(200)
      .json({
        message: 'InProcessQC entries fetched successfully',
        data: InProcessQCEntries,
      });
  } catch (error) {
    console.error('Error fetching InProcessQC entries:', error);
    res
      .status(500)
      .json({message: 'Failed to fetch InProcessQC entries', error});
  }
};
...
export const getInProcessQCById = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({
        message: 'InProcessQC entry fetched successfully',
        data: InProcessQCEntry,
      });
  } catch (error) {
    console.error('Error fetching InProcessQC entry by ID:', error);
    res
      .status(500)
      .json({message: 'Failed to fetch InProcessQC entry by ID', error});
  }
};
...
export const deleteInProcessQCById = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({
        message: 'InProcessQC entry deleted successfully',
        data: deletedInProcessQC,
      });
  } catch (error) {
    console.error('Error deleting InProcessQC entry:', error);
    res
      .status(500)
      .json({message: 'Failed to delete InProcessQC entry', error});
  }
};
