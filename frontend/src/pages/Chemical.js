...
const Chemical = () => {
  ...
  useEffect(() => {
    const fetchChemical = async () => {
      setLoading(true);
      try {
        const response = await apiClient.get('/chemical/getAllChemicals');
        setChemicals(response.data || []);
      } catch (error) {
        console.error("Error fetching chemical:", error);
        message.error("Failed to fetch chemical.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleDelete = async (id) => {
    try {
      await apiClient.delete(`/chemical/deleteChemical/${id}`);
  ...
  const filteredChemical = chemicals.filter((chemical) => {
    const statusMatch = status === "" || chemical.status.toString() === status;
    return statusMatch;
  });
        ...
        <div className="flex gap-3 items-center mt-2 md:mt-0">
          ...
          <Link to="/addChemical">
            <button className="bg-themeGradient hover:bg-themeGradientHover text-white px-2 py-1 rounded-md mb-4 md:mb-0 flex items-center">
              ...
              <span className="font-semibold ms-2">Add Chemical</span>
            </button>
          </Link>
        </div>
      </div>
                      ...
                      <td className="py-4 px-4 space-x-2">
                        <div className="flex gap-2 justify-center">
                          ...
                          <Popconfirm
                            ...
                            okText="Yes"
                            cancelText="No"
                            onConfirm={() => handleDelete(chemical._id)}
                          >
                  ...
                  <tr>
                    <td colSpan="3">
                      <div className="flex flex-col items-center justify-center my-10">
                        ...
                        <p className="text-gray-400 mt-4">No chemical found</p>
                      </div>
                    </td>
                  </tr>
                )}
