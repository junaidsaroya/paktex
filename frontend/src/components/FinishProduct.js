...
const FinishProduct = () => {
  ...
  const generateQCNumber = (finishProductList) => {
    ...
    if (finishProductList.length > 0) {
      const lastQcNumber =
        finishProductList[finishProductList.length - 1].qcNumber;
          ...
          <div className="overflow-x-auto overflow-hidden rounded-lg border border-gray-200">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody>
                ...
                ) : finishProductList.length > 0 ? (
                  finishProductList.map((finishProduct) => (
                    <tr
                      key={finishProduct._id}
                    ...
                    >
                      <td className="py-4 px-4">{finishProduct.productName}</td>
                      <td className="py-4 px-4">{finishProduct.batchNumber}</td>
                      <td className="py-4 px-4">{finishProduct.qcNumber}</td>
                      <td className="py-4 px-4">{finishProduct.qcAnalyst}</td>
